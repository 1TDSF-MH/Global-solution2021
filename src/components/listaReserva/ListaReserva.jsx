...
export default function ListaReserva() {
  ...
  useEffect(() => {
    fetch("/rest/reserva/produtos/" + assocId)
      .then((resp) => {
        return resp.json();
      })
      .then((resp) => {
  ...
  useEffect(() => {
    fetch("/rest/reserva/cdprodutos/" + assocId)
      .then((resp) => {
        return resp.json();
      })
      .then((resp) => {
  ...
  const confirmarReserva = () => {
    window.location = "/resumo/" + assocId;
  };
  ...
  return (
    <div>
      ...
      <table className="tab">
        ...
        <tbody>{produtos.map((c, i) => tabelaProdutos(c, i))}</tbody>
      </table>
      <div>
        <button title="confirmar" onClick={confirmarReserva} className="btn">
          Confirmar reserva
        </button>
